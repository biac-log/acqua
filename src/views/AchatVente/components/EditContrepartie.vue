        ...
        <v-card id="editContrepartie">
          ...
          <v-card-actions class="text-center ma-0">
            ...
            <v-tooltip top open-delay="500" open-on-hover>
              ...
              <span
                >Valider la contrepartie
                <span class="shortcutTooltip">alt + enter</span></span
              >
            </v-tooltip>
...
export default class extends Vue {
  ...
  private async searchCompteChanged(matchCode: string) {
    try {
      ...
      if (matchCode && matchCode.isInt()) {
        this.comptesSearch = await CompteApi.autocompleteCompteByNumero(
          this.typesComptesSelected.id,
          matchCode,
          5
        );
  ...
  private calculMontant() {
    if (this.caseTva.typeCase == 50)
      this.montant = this.calculMontantTva().toDecimalString(
        this.devisesSelected.typeDevise == "E" ? 0 : 2
      );
    else if (this.caseTva.typeCase == 1)
      this.montant = this.calculMontantTaxable().toDecimalString(
        this.devisesSelected.typeDevise == "E" ? 0 : 2
      );
  }
  private calculMontantTva(): number {
    let montantTva = this.tvaCalcule - this.tvaImpute;
    if (this.typesMouvementsSelected.id == "DB")
      montantTva = this.tvaImpute - this.tvaCalcule;
