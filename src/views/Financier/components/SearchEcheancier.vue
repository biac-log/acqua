    ...
    <v-card :loading="isLoading">
      ...
      <v-card-text class="pb-0">
        <v-row dense class="ml-1">
          ...
          <v-checkbox label="Uniquement les pièces non soldées" class="pa-0 ma-0 ml-5 mt-1" v-model="isNotSolde" hide-details></v-checkbox>
      ...
      <v-card-actions>
        <span class="ml-5">Reste à ventiler : <b>{{ resteAVentile | numberToStringEvenZero }}</b></span>
        <v-spacer></v-spacer>
...
export default class extends Vue {
  ...
  private headersEcheanciers = [
    ...
    { headerName: "Montant", field: "montantDevise", filter:true, width: 140, type: 'numericColumn', valueFormatter: this.montantDeviseToString },
    { headerName: "Montant", field: "montantBase",  filter:true, width: 140, type: 'numericColumn', valueFormatter: this.montantBaseToString, hide:true},
    ...
    { headerName: "Solde", field: "soldeDevise", filter:true, width: 140, type: 'numericColumn', valueFormatter: this.montantDeviseToString },
    { headerName: "Solde", field: "soldeBase", filter:true, width: 140, type: 'numericColumn', valueFormatter: this.montantBaseToString, hide:true },
    ...
    postSort: function(rowNodes){
      ...
      for (var i = 0; i < rowNodes.length; i++) {
        if(rowNodes[i].data.isPiecePrincipale){
          let piecePrincipal = rowNodes[i].data as EcheancierElement;
          let childs = rowNodes.filter(r => r.data.numeroJournalPrincipal == piecePrincipal.numeroJournal && r.data.numeroPiecePrincipal == piecePrincipal.numeroPiece && !r.data.isPiecePrincipale);
  ...
  private refreshGridElements() {
    if(this.allEcheanciers) {
      let elements : EcheancierElement[] = [] ;
      if(this.detailIsVisible) {
        _(this.allEcheanciers.elements).groupBy(e => `${e.numeroJournalPrincipal}${e.numeroPiecePrincipal}`)
  ...
  private calculAVentile() {
    ...
    if(!rowSelected?.length)
      ...
      this.resteAVentile = this.montantAVentileBase - Math.abs(_(rowSelected).map(r => r.data.montantDevise).sum());
    }else{
      this.resteAVentile = this.montantAVentileBase - Math.abs(_(rowSelected).map(r => r.data.montantBase).sum());
    }
  }
  ...
  private montantBaseToString(params: ValueFormatterParams): string {
    if(params.value)
      return `${(params.value as number).toComptaString(2)} USD`;
    else return "";
  }
