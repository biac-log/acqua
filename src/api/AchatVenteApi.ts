...
export abstract class AchatVenteApi {
  ...
  static async getPeriodes(): Promise<PeriodeComptable[]>{
    let responseCourante = await api.AcQuaCore.get<PeriodeComptableDTO>(`/AchatVente/GetPeriodeCourante`);
    let responsePrecedente = await api.AcQuaCore.get<PeriodeComptableDTO>(`/AchatVente/GetPeriodePrecedente`);
  ...
  static async GetEntetePiecesComptables(numeroJournal: number, periode: string, pagination: Pagination): Promise<PaginationResult<EntetePieceComptable>>{
    let response = await api.AcQuaCore.get<PaginationResult<EntetePieceComptable>>(`/AchatVente/GetEntetePiecesComptables?Page=${pagination.page}&Limit=${pagination.limit}&Term=${pagination.terms}&SortBy=${pagination.sortBy}&SortByAsc=${pagination.sortDesc}&NumeroJournal=${numeroJournal}&Periode=${periode}`);
  ...
  static async getDateEcheance(
    ...
    let response = await api.AcQuaCore.get<string>(
  ...
  static async getPieceComptable(
    ...
    let response = await api.AcQuaCore.get<PieceComptableDTO>(
  ...
  static async getTypesComptes(): Promise<TypeCompte[]> {
    let response = await api.AcQuaCore.get<TypeCompteDTO[]>(
      `/AchatVente/GetTypesCompteContreparties`
    );
  ...
  static async getComptesContreparties(): Promise<CompteContrepartieSearch[]> {
    let response = await api.AcQuaCore.get<
      CompteContrepartieSearchDTO[]
    >(
      `/AchatVente/GetTypesCompteContreparties`
    );
  ...
  static async getAllStatut(): Promise<Statut[]> {
    let response = await api.AcQuaCore.get<Statut[]>(`AchatVente/GetStatuts`);
    return response.data;
  }

  static async getCompteTva(numeroJournal: number | string, codeTaxe: string | number, codePays?: string): Promise<CompteGeneralSearch> {
    let response = await api.AcQuaCore.get<CompteGeneralSearch>(`/AchatVente/GetCompteTVA?numeroJournal=${numeroJournal}&codePays=${codePays}&codeTaxeCompteTiers=${codeTaxe}`);
    return response.data;
  }

  static async AddPiece(pieceComptable: PieceComptableSaveDTO): Promise<number> {
    let response = await api.AcQuaCore.post<number>(`/AchatVente/AddPieceComptable`, pieceComptable);
    return response.data;
  }

  static async UpdatePiece(pieceComptable: PieceComptableSaveDTO): Promise<PieceComptableSaveDTO> {
    let response = await api.AcQuaCore.put<PieceComptableSaveDTO>(`/AchatVente/UpdatePieceComptable`, pieceComptable);
    return response.data;
  }

  static async DeletePiece(periode: string, journal: number, piece: number): Promise<any> {
    let response = await api.AcQuaCore.delete<any>(`/AchatVente/DeletePieceComptable?periode=${periode}&journal=${journal}&piece=${piece}`);
    return response.data;
  }

  static async ValidateLibelle(libelle: string, typeComte: string, numeroCompte: string | number): Promise<boolean>{
    let response = await api.AcQuaCore.get<boolean>(`/AchatVente/IsReferenceExiste?typeCompte=${typeComte}&numeroCompte=${numeroCompte}&reference=${libelle}`);
    return response.data;
  }

  static async ChangeNumero(periode: string, journal: number, oldNumero: number, newNumero: number): Promise<boolean>{
    await api.AcQuaCore.put(`/AchatVente/ChangementNumero?periode=${periode}&numeroJournal=${journal}&ancienNumeroPiece=${oldNumero}&nouveauNumeroPiece=${newNumero}`);
    return true;
  }
}